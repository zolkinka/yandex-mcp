...
async function testWithHeader(headerName, orgId) {
    console.log(`\n🔄 Тестируем с заголовком: ${headerName}: ${orgId}`);
    try {
        const response = await axios_1.default.get(`${config_1.config.YANDEX_TRACKER_BASE_URL}/myself`, {
            headers: {
...
async function testCredentials() {
    ...
    const orgId = config_1.config.YANDEX_TRACKER_ORG_ID || config_1.config.YANDEX_TRACKER_CLOUD_ORG_ID || "";
    // Проверка переменных окружения
    console.log("\n📋 Текущие настройки:");
    console.log(`   Token: ${config_1.config.YANDEX_TRACKER_TOKEN?.substring(0, 20)}...`);
    console.log(`   Org ID: ${orgId}`);
    console.log(`   Base URL: ${config_1.config.YANDEX_TRACKER_BASE_URL}`);
